...
{
    public static void Render(ILogger logger, QudiVisualizationReport report, LoggerOutput output)
    {
        if (output == LoggerOutput.None)
        {
            return;
        }

        if (IsEnabled(output, LoggerOutput.Summary))
        {
        ...
        if (IsEnabled(output, LoggerOutput.List))
        {
            logger.LogDebug(
        ...
        if (IsEnabled(output, LoggerOutput.Issues))
        {
    ...
    private static bool IsEnabled(LoggerOutput output, LoggerOutput flag)
    {
